...
let (|Empty|SimpleOnly|MappedWindow|SingleComposite|UnmappedWindow|MixedSingleAndComposite|Unknown|) modelConstructions =
    ...
    | [|Composite(name, srcs)|] -> SingleComposite(name, srcs |> Array.map (fun (src, thickness) -> (src.MappingTarget, thickness)))
    | _ ->
        ...
        let asSimple = Array.map (fun m ->
            match m with
            | noMapping when noMapping = Unchecked.defaultof<LibraryEntry> -> Some(Unchecked.defaultof<LibraryEntry>)
            | Opaque(_) | AirGap(_) -> Some(m)
            | _ -> None) mapped
        if not (Array.exists (fun (maybe:LibraryEntry option) -> maybe.IsNone) asSimple) then SimpleOnly(Array.choose id asSimple) else
        Unknown