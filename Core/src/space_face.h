...
public:
	space_face(const space * sp, const oriented_area & geometry) 
		: m_sense(geometry.sense()), 
		  m_height(geometry.height()), 
		  m_area(geometry.area_2d()), 
		  m_orig_reg_area(geometry.area_2d().regular_area()),
		  m_space(sp) { }

	bool sense() const { return m_sense; }
	const NT & height() const { return m_height; }
	const area & face_area() const { return m_area; }
	const NT & starting_regular_area() const { return m_orig_reg_area; }
