...
std::map<const orientation *, std::vector<const block *>> get_blocks_by_orientation(const BlockRange & blocks) {
	...
	boost::for_each(blocks, [&res](const block & b) { res[b.o()].push_back(&b); });
	return res;
}
...
void process_group(SpaceFaceRange * space_faces, const BlockRange & blocks, double height_cutoff, double height_eps, OutputIterator oi) {
	stacking_graph g = create_stacking_graph(space_faces, blocks, height_eps, height_cutoff);
	...
	boost::for_each(
		...
		[&g, height_cutoff, height_eps, &oi](stacking_vertex starting_face) { begin_traversal(g, starting_face, height_cutoff, height_eps, oi); });
}
...
std::vector<blockstack> build_stacks(const BlockRange & blocks, const SpaceRange & spaces, double height_cutoff, equality_context * c) {
	...
	auto fenestration_blocks = get_blocks_by_orientation(blocks | boost::adaptors::filtered([](const block & b) { return b.is_fenestration(); }));
	auto nonfenestration_blocks = get_blocks_by_orientation(blocks | boost::adaptors::filtered([](const block & b) { return !b.is_fenestration(); }));

	std::vector<blockstack> res;
	boost::for_each(space_faces, [&res, &space_faces, &fenestration_blocks, height_cutoff, c](std::pair<const orientation *, std::vector<space_face>> & o_info) {
		process_group(&o_info.second, fenestration_blocks[o_info.first], height_cutoff, c->height_epsilon(), std::back_inserter(res));
	});
	boost::for_each(space_faces, [](std::pair<const orientation *, std::vector<space_face>> & o_info) {
		boost::for_each(o_info.second, [](space_face & f) { f.reset_area_to_original(); });
	});
	boost::for_each(space_faces, [&res, &space_faces, &nonfenestration_blocks, height_cutoff, c](std::pair<const orientation *, std::vector<space_face>> & o_info) {
		process_group(&o_info.second, nonfenestration_blocks[o_info.first], height_cutoff, c->height_epsilon(), std::back_inserter(res));
	});
